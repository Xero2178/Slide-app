    ...
    return(
        <div className="mx-0 px-4 grid">
            <div className="flex gap-4">
                {reactFact.map((fact) => <Tab id={fact.id} onSetSelected={handleSetSelected} selected={selected}/>)}
            </div>
            <div>
                <TabContent selected={selected} array={array} onSetArray={handleSetArray}/>
            </div>
...
function TabContent({selected, array, onSetArray}) {
    ...
    return(
        <div className="w-3/4 max-w-xl  border-2 border-blue-600  p-4 rounded-xl bg-sky-200">
            <div className="mb-5">
            <h1 className="font-bold text-2xl mb-5 text-sky-700">{array[selected].fact}</h1>
            <p>{array[selected].detail}</p>
            </div>
